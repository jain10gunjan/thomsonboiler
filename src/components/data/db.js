import image1 from "../images/check1.png";
import image6 from "../images/71.png";
import image2 from "../images/9.png";
import image3 from "../images/10.png";
import image4 from "../images/11.png";
import image5 from "../images/12.png";

export const productsData = [
  {
    id: 1,
    productName: "OIL/GAS FIRED",
    slug: "oil-gas-fired",
    alter: false,
    tagLine:
      "Used to burn oil and gas to produce heat or steam for industrial applications",
    sailentFeatures: [
      {
        point: "Compact design with minimum foundation/civilwork",
      },
      {
        point: "Fully automatic, wet back, 3-pass smoke tube design",
      },
      {
        point:
          "Option available with both indian and imported monobloc/dualbloc burner.",
      },
      {
        point:
          "More stream space. high dryness fraction and better response influcating load.",
      },
      {
        point: "High combustion efficiency.",
      },
    ],
    technicalSpecification: {
      productRange: "0.3TPH~20TPH",
      pressure: "10.54 Kg/Cm<sup>2</sup> to 32 Kg/Cm<sup>2</sup>",
    },
    fuelUsed: [
      {
        fuel: "PNG/LPG",
        imgurl:
          "https://w7.pngwing.com/pngs/593/411/png-transparent-lpg-gas-red-gas-bottle-thumbnail.png",
      },
      {
        fuel: "Light Diesel Oil (LDO)",
        imgurl:
          "https://5.imimg.com/data5/SELLER/Default/2023/5/305436547/GB/TE/FI/3731739/ldo-light-diesel-oil.jpg",
      },
      {
        fuel: "High Speed Diesel Fuel (HSD)",
        imgurl:
          "https://5.imimg.com/data5/SELLER/Default/2024/3/402992976/GY/OI/SC/113023594/high-speed-diesel-hsd.jpg",
      },
    ],
    imgSrc: image1.src,
  },
  {
    id: 2,
    productName: "Thompac",
    slug: "thompac",
    alter: true,
    tagLine:
      "Ideal for burning coal, briquettes,wood waste, and imported coal.",
    sailentFeatures: [
      {
        point: "Optimally designed for coal firing/ briqutte/wood waste.",
      },
      {
        point:
          "Construction as per latest IBR or any other standard code of construction like ASME, BS, DIN etc.",
      },
      {
        point:
          "Optimum steam space, furnace volume & well designed Grate area & bars.",
      },
      {
        point:
          "Its fully wet back design reduces the heat loss due to Radiation resulting in better efficiency.",
      },
      {
        point:
          "The inbuilt grate bars are specifically designed for type of fuel: Coal/Briquette/Wood waste. 'Shanti' also has specially designed grate bars for pulverised & imported coal.",
      },
      {
        point:
          "Optimum steam space with unique steam separator resulting in dry steam.",
      },
      {
        point:
          "The boilers require minimum foundation for quick installation and commissioning.",
      },
      {
        point:
          "High efficient vertical stainless steel pumps. Radiation Shield on Front & Rear for the first time designed to provide Air Insulation.",
      },
      {
        point: "Efficiency ranging from 82% (+2%)",
      },
      {
        point:
          "All boilers pass through strict quality control tests like stress relieving, radiography, and other NDT tests.",
      },
    ],
    technicalSpecification: {
      productRange: "2TPH~6TPH",
      pressure: "10.54/17.5/21 Kg/Cm<sup>2</sup>",
    },
    fuelUsed: [
      {
        fuel: "Wood",
        imgurl:
          "https://5.imimg.com/data5/SELLER/Default/2021/12/HW/KG/SQ/143354869/syp-wood-timber-plank.jpg",
      },
      {
        fuel: "Coal",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Bituminous_Coal.JPG/1200px-Bituminous_Coal.JPG",
      },
      {
        fuel: "Briquette",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/f/f2/Charcoal_Briquette.JPG",
      },
    ],
    imgSrc: image6.src,
  },
  {
    id: 3,
    productName: "THOM MAXX",
    slug: "thom-maxx",
    alter: false,
    tagLine:
      "High thermal efficiency with three pass design and fluidized bed combustion.",
    sailentFeatures: [
      {
        point:
          "High thermal efficiency with three pass design and fluidized bed combustion.",
      },
      {
        point: "Flexible design with three or four side waterwalls",
      },
      {
        point: "Large capacity for water, steam, and load response",
      },
      {
        point:
          "Large tubes to prevent choking. Economizer for heat recovery and fuel savings.",
      },
      {
        point: "Low fuel use and maintenance costs.",
      },
    ],
    technicalSpecification: {
      productRange: "2TPH~6TPH",
      pressure: "10.54/17.5/21 Kg/Cm<sup>2</sup>",
    },
    fuelUsed: [
      {
        fuel: "Rice Husk",
        imgurl:
          "https://www.alkarty.com/images/product/resized/500-500/08-10-202151m7BbaCQTL.jpg",
      },
      {
        fuel: "Pet Coke",
        imgurl:
          "https://vajiram-prod.s3.ap-south-1.amazonaws.com/petcoke_d867bc8273.jpg",
      },
      {
        fuel: "Coal",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Bituminous_Coal.JPG/1200px-Bituminous_Coal.JPG",
      },
      {
        fuel: "Saw Dust",
        imgurl:
          "https://m.media-amazon.com/images/I/61cWtrnrLiL._AC_UF1000,1000_QL80_.jpg",
      },
    ],
    imgSrc: image3.src,
  },
  {
    id: 4,
    productName: "COMBI-THOM",
    slug: "combi-thon",
    alter: true,
    tagLine:
      "A boiler with a water tube, a smoke tube, and sufficient refractory work combined.",
    sailentFeatures: [
      {
        point:
          "A boiler with a water tube, a smoke tube, and sufficient refractory work combined.",
      },
      {
        point: "A larger furnace volume with a wide range of fuel.",
      },
      {
        point:
          "Longer life and greater efficiency are ensured by the unique four-, two-, and five-side water wall designs,",
      },
      {
        point:
          "which offer stronger heat transfer in the radiation zone and lower flue gas entry temperature in the convective zone.",
      },
      {
        point:
          "Combuster that can be used to fire a variety of solid fuel sources, including horizontal reciprocating grate, bubbling bed, fluidized bed, dumping grate, and pulsating grate.",
      },
      {
        point:
          "A furnace partition to accommodate various fuel feeding configurations.",
      },
      {
        point: "Efficiency as high as 82% ± 2% *",
      },
      {
        point: "Above-and below-the-bed feeding arrangements .",
      },
      {
        point: "In-bed coil configuration.",
      },
      {
        point: "Arrangement for submerged ash handling unit",
      },
    ],
    technicalSpecification: {
      productRange: "0.5~16 TPH",
      pressure: "10.54/17.5/21 Kg/cm",
    },
    fuelUsed: [
      {
        fuel: "Wood",
        imgurl:
          "https://5.imimg.com/data5/SELLER/Default/2021/12/HW/KG/SQ/143354869/syp-wood-timber-plank.jpg",
      },
      {
        fuel: "Biomass",
        imgurl:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRF97LpvatcvKKJkse7k0lu3iusumEZtyx1A&s",
      },
      {
        fuel: "Coal",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Bituminous_Coal.JPG/1200px-Bituminous_Coal.JPG",
      },
      {
        fuel: "Saw Dust",
        imgurl:
          "https://m.media-amazon.com/images/I/61cWtrnrLiL._AC_UF1000,1000_QL80_.jpg",
      },
      {
        fuel: "Biomass Fuels",
        imgurl:
          "https://media.istockphoto.com/id/1175666433/photo/small-pile-of-wood-pellets-with-on-top-of-green-leaves.jpg?s=612x612&w=0&k=20&c=D6JU33Fo7HqZBjqrT_E5v8QVCr4LUtxUiYqa5Hth88M=",
      },
      {
        fuel: "Wood Chips",
        imgurl: "https://5.imimg.com/data5/JG/LK/MY-17255263/wood-chips.jpg",
      },
      {
        fuel: "Rice Husk",
        imgurl:
          "https://www.alkarty.com/images/product/resized/500-500/08-10-202151m7BbaCQTL.jpg",
      },
    ],
    imgSrc: image4.src,
  },
  {
    id: 5,
    productName: "WATER TUBE BOILER",
    slug: "water-tube-boiler",
    alter: false,
    tagLine:
      "Efficient Water Tube Design for AFBC, Travelling Grate, Reciprocating Grate, and Dumping Grate combustors",
    sailentFeatures: [
      {
        point:
          "Efficient Water Tube Design for AFBC, Travelling Grate, Reciprocating Grate, and Dumping Grate combustors",
      },
      {
        point: "Low Maintenance & Operating Costs for long-term savings",
      },
      {
        point: "Large Heat Transfer Area for better steam evaporation.",
      },
      {
        point: "Optimized Combustion for maximum heat transfer efficiency.",
      },
      {
        point:
          "Consistent Steam Output with minimal power consumption, ideal for co-generation to reduce electricity costs.",
      },
    ],
    technicalSpecification: {
      productRange: "6TPH~25TPH",
      pressure: "10.54/17.5/28/32/40 Kg/Cm<sup>2</sup>",
    },
    fuelUsed: [
      {
        fuel: "Mustard Husk",
      },
      {
        fuel: "Paddy Straw",
        imgurl:
          "https://5.imimg.com/data5/SELLER/Default/2020/12/ZD/EO/BW/97922368/rice-paddy-straw.jpg",
      },
      {
        fuel: "Coal",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Bituminous_Coal.JPG/1200px-Bituminous_Coal.JPG",
      },
      {
        fuel: "Bagasse",
        imgurl: "https://cdn.britannica.com/31/126031-050-8A05CA97/Bagasse.jpg",
      },
      {
        fuel: "Wood Waste",
        imgurl:
          "https://5.imimg.com/data5/ANDROID/Default/2023/5/308728358/AE/PS/NS/40127620/product-jpeg-500x500.jpg",
      },
      {
        fuel: "Rice Husk",
        imgurl:
          "https://www.alkarty.com/images/product/resized/500-500/08-10-202151m7BbaCQTL.jpg",
      },
    ],
    imgSrc: image5.src,
  },
  {
    id: 6,
    productName: "Thompac Plus",
    slug: "thompac-plus",
    alter: true,
    tagLine: "Superior compact design enables minimum space requirement.",
    sailentFeatures: [
      {
        point: "Superior compact design enables minimum space requirement.",
      },
      {
        point: "High thermal efficiency with 3 pass design and integral APH. ",
      },
      {
        point:
          "Factory manufactured and assembled enables minimum site work requirement.",
      },
      {
        point:
          "Large steam water interface area for greater dryness fraction. Large steam water interface area for greater dryness fraction.",
      },
      {
        point:
          "Skid-mounted design reduces civil foundation and installation costs.",
      },
    ],
    technicalSpecification: {
      productRange: "2TPH~6TPH",
      pressure: "10.54/17.5/21 Kg/Cm<sup>2</sup>",
    },
    fuelUsed: [
      {
        fuel: "Wood",
        imgurl:
          "https://5.imimg.com/data5/SELLER/Default/2021/12/HW/KG/SQ/143354869/syp-wood-timber-plank.jpg",
      },
      {
        fuel: "Coal",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Bituminous_Coal.JPG/1200px-Bituminous_Coal.JPG",
      },
      {
        fuel: "Briquette",
        imgurl:
          "https://upload.wikimedia.org/wikipedia/commons/f/f2/Charcoal_Briquette.JPG",
      },
    ],
    imgSrc: image2.src,
  },
];
